void GetRunInfo_PIXELhitRates(const char *fileName, unsigned int runLow = 0, unsigned int runUp = 0) {
  ...
  int nTotalEvents = 0, nTotalTracks = 0, nTotalHits = 0, nRunsUsed = 0;
  ...
  for (Long64_t jentry = 0; jentry < n; jentry++)  // Loop over Runs
  {
    ...
    fprintf(pFile,
	    "%u,%-7.0lf,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i\n",
            runnum,
	    run_time,
	    number_of_events,
	    number_of_tracks,
